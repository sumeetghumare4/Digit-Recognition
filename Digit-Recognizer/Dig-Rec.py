...
def main():
    ...
    # Setup camera and processing variables
        ...
        if len(cnts) >= 1:
            ...
            if len(pts) != 0:
                ...
                if len(blackboard_cnts) >= 1:
                    ...
                    print("No contours found on the blackboard.")

            pts = deque(maxlen=512)
            blackboard = np.zeros((480, 640, 3), dtype=np.uint8)

        # Display results
        cv2.putText(img, "Logistic Regression : " + str(ans1), (10, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(img, "Shallow Network :  " + str(ans2), (10, 440), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(img, "Deep Network :  " + str(ans3), (10, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
